...
if ( ! function_exists( $func_repo_exists ) ) {
	eval( 'function ' . $func_repo_exists . '() {
...
if ( ! function_exists( $func_check_update ) ) {
	eval( 'function ' . $func_check_update . '( $transient ) {
		global $PLUGIN_SLUG, $GITHUB_REPO_URL, $PACKAGE_FILE;
		if ( empty($transient->checked) ) return $transient;
		if ( ! ' . $func_repo_exists . '() ) return $transient;
		$current_version = ' . $func_local_ver . '();
		$remote_version  = ' . $func_latest_rel . '();
...
if ( ! function_exists( $func_plugin_info ) ) {
	eval( 'function ' . $func_plugin_info . '( $res, $action, $args ) {
		...
		if ( ! ' . $func_repo_exists . '() ) return $res;
		$remote_version = ' . $func_latest_rel . '();
